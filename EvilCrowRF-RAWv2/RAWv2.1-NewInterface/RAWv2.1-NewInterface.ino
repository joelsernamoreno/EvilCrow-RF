...
// HTML and CSS style
const String MENU = "<body><p>Evil Crow RF v1.0</p><div id=\"header\"><body><nav id='menu'><input type='checkbox' id='responsive-menu' onclick='updatemenu()'><label></label><ul><li><a href='/'>Home</a></li><li><a class='dropdown-arrow'>Config</a><ul class='sub-menus'><li><a href='/txconfig'>RAW TX Config</a></li><li><a href='/txbinary'>Binary TX Config</a></li><li><a href='/rxconfig'>RAW RX Config</a></li><li><a href='/btnconfig'>Button TX Config</a></li></ul></li><li><a class='dropdown-arrow'>RX Log</a><ul class='sub-menus'><li><a href='/viewlog'>RX Logs</a></li><li><a href='/delete'>Delete Logs</a></li></ul></li></ul></nav><br></div>";
...
void setup() {
  ...
  controlserver.on("/btnconfig", []() {
    controlserver.send_P(200, "text/html", ButtonConf);
  });

  controlserver.on("/settx", [](){
    raw_rx = "0";
    tmp_module = controlserver.arg("module");
  ...
  controlserver.on("/setbtn", [](){
    btn_set = controlserver.arg("button");
    btn_set_int = btn_set.toInt();
    
    if (btn_set_int == 1){
      btn1_rawdata = controlserver.arg("rawdata");
      btn1_deviation = controlserver.arg("deviation");
      btn1_frequency = controlserver.arg("frequency");
      btn1_mod = controlserver.arg("mod");
      btn1_transmission = controlserver.arg("transmissions");
      counter=0;
      int pos = 0;
    ...
    if (btn_set_int == 2){
      btn2_rawdata = controlserver.arg("rawdata");
      btn2_deviation = controlserver.arg("deviation");
      btn2_frequency = controlserver.arg("frequency");
      btn2_mod = controlserver.arg("mod");
      btn2_transmission = controlserver.arg("transmissions");
      counter=0;
      int pos = 0;
  ...
  controlserver.on("/viewlog", [](){
    ...
    logs.close();
    logs = SPIFFS.open("/logs.txt", "a+");
    controlserver.send(200, "text/html", HTML_CSS_STYLING+"<head><meta http-equiv=\"refresh\" content=\"5\"></head>" +"View Log: "+serverlog +"\n-----\n"+webString+"</pre></h2>");
  });

  controlserver.on("/delete", [](){
    logs.close();
    logs = SPIFFS.open("/logs.txt", "w");
    controlserver.send(200, "text/html", HTML_CSS_STYLING+ "<body onload=\"JavaScript:AutoRedirect()\">"
  ...
  controlserver.on("/lib.js", HTTP_GET, []() {
