...
String defaultSSID = "Evil Crow RF";  // Enter your SSID here
String defaultPassword = "123456789ECRFv1";  //Enter your Password here
...
void handleUpdateWiFi(AsyncWebServerRequest *request) {
  if (request->hasParam("ssid", true) && request->hasParam("password", true)) {
    ...
    appendFile(LittleFS, "/wifi_config.txt", newSSID.c_str(), "\n");
    appendFile(LittleFS, "/wifi_config.txt", newPassword.c_str(), "\n");
...
void printReceived() {
  ...
  //Serial.println();
  OutputLog += "\n";
  appendFile(LittleFS, "/logs.txt", NULL, OutputLog.c_str());
}
...
void signalanalyse(){
  ...
  appendFile(LittleFS, "/logs.txt", NULL, "\n");
...
void setup() {
  ...
  controlserver.on("/rxconfig", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/html", RXConfig);
  });

  controlserver.on("/viewlog", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", ViewLog);
  });
  ...
  controlserver.on("/txconfig", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/html", TXConfig);
  });
  ...
  controlserver.on("/config", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "text/html", Config);
  });
